...
function BookTeacher() {
  ...
  const { mutate: createBooking, isPending } = useMutation({
    ...
    onSuccess: () => {
      ...
      setBookOpen(false); setDate(""); setNote(""); setSelectedSlot(null)
    },
  ...
  const handleBook = () => {
    ...
    createBooking({
      body: { teacher_id: Number(teacherId), date, start_time: selectedSlot.start, end_time: selectedSlot.end, note },
    })
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={bookOpen} onOpenChange={(o) => { if (!o) setBookOpen(false) }}>
        <DialogContent>
          <DialogHeader><DialogTitle>Booking Les</DialogTitle></DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleBook} disabled={!date || isPending}>{isPending ? <Spinner /> : "Kirim Booking"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
